...
public class update_teacher extends JFrame implements ActionListener {
    ...
    JPanel p1, p2, p3;
    Font f;
    String x[] = {"Employ ID", "Name", "Father's Name", "Age", "DOB", "Address", "Phone", "Email", "Class X", "Class XII", "Branch", "Corece"};
    ...
    update_teacher() {
        ...
        setLocation(50, 50);
        setResizable(false);
        l1 = new JLabel("Enter Employ ID number to delete Teacher");
        ...
        try {
            ...
            String q = "select * from new_teacher_add";
    ...
    public void actionPerformed(ActionEvent ev) {
        String employId = tf.getText();

        if (ev.getSource() == bt1) {
            try {
                connection obj = new connection();
                String qq = "delete from new_teacher_add where employ_ID='" + employId + "'";
        ...
        if (ev.getSource() == bt3) {
            new updateTeacher_record(employId).setVisible(true);
            setVisible(false);
        }
    }
